...
(()=>{
    ...
    forms.forEach((form)=>{
        form.addEventListener("submit", (event)=>{
            ...
            if (!form.checkValidity()) preventRequest = true;
            event.preventDefault();
            form.classList.add("was-validated");
            const btnForm = document.querySelector("button");
            let name = document.querySelector(".inputName").value;
            let country = document.querySelector(".inputCountry").value;
            if (preventRequest == false) fetch("https://api.agify.io?name=" + name + "&country_id=" + country).then((response)=>response.json()).then((agifyResponse)=>{
                let age = agifyResponse.age;
                if (age == null) {
                    console.log("pas d'occurence dans ce pays");
                    console.log(agifyResponse);
                    fetch("https://api.agify.io?name=" + name).then((response)=>response.json()).then((secondAgifyResponse)=>{
                        document.querySelector("p").textContent = "Vous avez probablement " + secondAgifyResponse.age + " ans!";
                        console.log(secondAgifyResponse);
                    });
                } else {
                    document.querySelector("p").textContent = "Vous avez probablement " + agifyResponse.age + " ans!";
                    console.log(agifyResponse);
                }
            });
        }, false);
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9tRox","1SICI"], "1SICI", "parcelRequire16f5")
